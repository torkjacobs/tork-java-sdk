...
PIIDetector.DetectionResult r = detector.detectAndRedact(
    "South African ID number 8001015009087 for the FICA check.");

r.getRegions();       // ["ZA"]
r.getCountryLabels(); // ["ZA_ID"]
r.getRedactedText();  // "South African ID number [ZA_ID_REDACTED] for the FICA check."
```

Force profiles on when you already know the jurisdiction:

```java
PIIDetector.DetectionResult forced = detector.detectAndRedact(
    "Documento 529.982.247-25 arquivado.", List.of("br"));
// forced.getRedactedText() == "Documento [CPF_REDACTED] arquivado."
```
